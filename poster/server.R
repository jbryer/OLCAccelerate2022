shinyServer(function(input, output, session) {
	source('server_formative.R', local = TRUE)

	##### Figures ##############################################################
	output$mass_figure <- renderPlot({
		likert(mass) |> plot(legend = '')
	})

	output$omp_figure1 <- renderPlot({
		words <- omp[,'What was the most important thing you learned during this class?',drop=TRUE]
		# wordcloud::wordcloud(words, min.freq = 2)
		ompWordCloud(words)
	})

	output$omp_figure2 <- renderPlot({
